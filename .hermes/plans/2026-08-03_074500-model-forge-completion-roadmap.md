# Model Forge Completion Roadmap
...
> **Scope:** A gap analysis and pragmatic work-order plan to bring Model Forge
...
> Model Forge's stronger authority/storage/execution model.

**Goal:** Deliver a working, user-facing research orchestration system on
Model Forge that can run real Hermes agent research, with a UI experience at
...
- **Model Forge** (new baseline): 20,220 lines of backend Python + 7,857 lines
...
## Part 1 — What Model Forge Already Has (Stronger Than Research Hub)

These are architectural advantages Model Forge already holds. The roadmap
...
3. **RFC 8785 deterministic digests** — canonical JSON serialization with
   Unicode and numeric test vectors (`src/model_forge/digests/`).
...
## Part 2 — Feature Gap Analysis (What Model Forge Lacks)

### Category A: Production Hermes Execution (Critical — Nothing Runs Today)

| Gap | Research Hub (works) | Model Forge (missing/incomplete) |
|---|---|---|
...
| Feature | Research Hub | Model Forge |
|---|---|---|
...
| **Run approve / revision request** | `approve_run()`, `request_revision()` — researcher gates publication | Model Forge architecture **explicitly rejects** post-run approval (no automatic retry). This is a **deliberate design difference**, not a gap. |
| **Branch retire** | `retire_branch()` — retires a phase branch | Model Forge uses method lifecycle (retire/reactivate) instead — **deliberate redesign** |
...
### Category D: Domain Subsystems (Large — Research Hub Has 12K+ Lines Model Forge Lacks)

| Subsystem | Research Hub LOC | Model Forge Equivalent |
|---|---|---|
| **Knowledge graph** | 6,511 lines (`knowledge_graph.py`, `knowledge_fragments.py`, `knowledge_heads.py`, `knowledge_events.py`, `knowledge_content.py`, `knowledge_basis.py`, `knowledge_schema.py`, `knowledge_event_diff.py`) | **None.** Model Forge has no knowledge-graph or fragment-level knowledge management. The architecture mentions "knowledge resources" as a role-input category but does not implement a graph. |
...
| **Hermes profile existence verification** | 🔴 Critical | Model Forge cannot confirm the configured Hermes profiles actually exist on disk before launching. |
| **Bounded subprocess output** | 🟡 High | Research Hub's `_run_process_with_bounded_output()` caps stdout/stderr. Model Forge executor has `output_limit_bytes` check post-hoc but no streaming bound. |
| **Process-tree kill on timeout** | 🟡 High | Research Hub has Windows job-object + Linux process-group kill. Model Forge only calls `kanban archive`. |
...
while not skipping the integrity gates that makes Model Forge worth building.
...
0.1. **Hermes profile verification** — before any run, confirm the configured
     ...
     - Files: `src/model_forge/configuration/profiles.py`,
       `src/model_forge/configuration/resources.py`
...
0.2. **Bounded output capture** — port Research Hub's
     ...
     - Files: `src/model_forge/executors/hermes.py`,
...
0.4. **Run log storage and viewer** — store captured output as an immutable
     ...
     - Files: `src/model_forge/api/router.py`, `web/src/pages/RunPage.tsx`
...
0.5. **Enable `hermes_kanban` for a real single-role test** — with
     ...
     end-to-end execution on Model Forge.
...
1.1. **Seal input generation IDs and digests** in the accepted `RunCommand`.
     ...
     - Files: `src/model_forge/harness/commands.py`,
       `src/model_forge/harness/preparation.py`, `src/model_forge/domain/runs.py`
...
where Model Forge starts doing actual research.
...
2.4. **Phase 4 pilot** — run a real empirical phase. Validate: 4-slot atomic
     ...
     - Files: extend `harness/submission_validation.py` with Phase 4-specific
       ...
       contract + validation adapter is the Model Forge way.
...
2.6. **Negative-result fixture** — validate that a complete-but-negative
     scientific result publishes correctly. This is a core Model Forge
     invariant.
...
3.3. **Profile memory viewer** — port Research Hub's
     ...
     - Files: `src/model_forge/api/router.py`,
       `src/model_forge/application/profile_views.py`,
...
3.5. **Admin recovery controls** — add a safe UI action for inspecting a stuck
     run and triggering operational cleanup (the Model Forge equivalent of
...
Research Hub has a 6,511-line knowledge graph subsystem. Model Forge's
...
- **Legacy Research Hub importer** (WP8) — the entangled Langevin project
  should be re-run natively on Model Forge rather than imported.
- **Supported deployment / OS qualification** (WP7) — this machine is the
  deployment target for now.

---

## Part 4 — Summary: What Makes Model Forge "Complete"

A functioning Model Forge that matches Research Hub's utility needs:
...
Throughout implementation, these Model Forge principles must not be violated:
...
2. **No post-run approval gate.** Model Forge deliberately rejects Research
...
4. **No dual-writing.** Model Forge must not import Research Hub code, use its
...
*Prepared: 2026-08-03*
*Baseline: model-forge@a0798ef (244 tests), research-hub HEAD (893 tests)*
