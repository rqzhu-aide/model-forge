...
Permission enforcement occurs through the capability broker defined in the role-context contract. A role process receives no project-store or formal-store credentials. When it can invoke arbitrary filesystem or command-line tools, the supported-platform process sandbox must also deny path, link, subprocess, network, secret, and direct-storage escape. Prompt instructions alone are insufficient.

## 6. Role execution and communication

The phase contract defines stage order, execution groups, and visibility. The sealed run manifest resolves that contract into a frozen recipe. Immediately before one role starts, the harness creates an immutable `PreparedRoleContext` and `RoleInvocationStart`. The start copies and hashes the exact manifest role-plan entry and binds the resolved profile, actual input artifacts, expected output contracts and paths, capability grants, rootless OCI executor profile, runtime image digest, write root, and start time. It contains no future output or terminal-status claim.
