...
5. [Phase run walkthroughs](02b-phase-run-walkthroughs.md) walks each phase P1-P5 through the formal lane: modes, role stages (parallel/serial), per-stage outputs and schemas, instruction templates, validators, and publication bindings, all sourced from the executable phase contracts.
6. [Storage and authority](03-storage-and-authority.md) defines immutable generations, append-only authority events, rebuildable current state, logical paths, and phase-specific storage semantics.
7. [UI contract](04-ui-contract.md) defines how formal records and user commands are projected into the Web interface.
...
10. `schemas/` contains 46 machine-validatable schemas, while `examples/` contains 58 valid examples and 16 focused invalid fixtures.
11. `scenarios/` defines 12 end-to-end acceptance cases, including failures, method changes, control commands, and [S12 disjoint concurrent publication](scenarios/S12-disjoint-concurrent-publication.md).
