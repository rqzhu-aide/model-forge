"""Validated deployment settings for the local Model Forge service."""
...
class ApplicationSettings(BaseSettings):
    model_config = SettingsConfigDict(
        env_prefix="MODEL_FORGE_",
        env_file=".env",
        extra="ignore",
    )

    data_root: Path = Field(default_factory=lambda: Path.home() / ".model-forge")
    architecture_root: Path | None = None
    host: str = "127.0.0.1"
    port: int = 8765
    user_id: str = "researcher.local"
    executor_kind: Literal["disabled", "fake", "hermes_kanban", "local_hermes"] = "disabled"
    ...
    #: is unaffected and cannot select the local_hermes executor.
    diagnostic_enabled: bool = False
    hermes_executable: str = "hermes"
    hermes_board: str = "model-forge"
